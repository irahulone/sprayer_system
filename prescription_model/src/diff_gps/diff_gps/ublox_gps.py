...
class UbloxGps(object):
    ...
    def request_standard_packet(self, cls_name, msg_name, ubx_payload=None, wait_time=2500, rethrow_thread_exception=True):
        prev = self.packets.get(cls_name, {}).get(msg_name)
    
        # If not auto, clear and poll
        ...
        while True:
            pkt = self.packets.get(cls_name, {}).get(msg_name)
            if pkt is not None:
                if (prev is None) or (getattr(pkt, "iTOW", None) != getattr(prev, "iTOW", None)):
                    break  # got a fresh one
            if (time.monotonic() - start) >= (wait_time / 1000.0):
                break
            time.sleep(0.005)
        ...
        return self.scale_packet(pkt) if pkt is not None else None
